  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (editingPost) {
        ...
        setSuccess(form.status === 'published' ? 'Публикация размещена на сайте.' : 'Черновик сохранён.');
      }
    ...
    >
      ...
      {editorOpen && (
        <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true" aria-labelledby="blog-editor-title">
          <button type="button" aria-label="Закрыть редактор" onClick={() => !saving && setEditorOpen(false)} className="absolute inset-0 bg-brand-navy/70 backdrop-blur-sm" />
          <div className="relative h-full w-full max-w-3xl overflow-y-auto bg-[#f8f9fb] shadow-2xl">
            <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-[#dfe4ec] bg-white px-5 py-5 sm:px-8">
              <div>
                <p className="text-xs font-semibold tracking-[0.16em] text-brand-orange uppercase">Редактор блога</p>
                <h2 id="blog-editor-title" className="mt-1 text-2xl font-extrabold text-brand-navy">{editingPost ? 'Редактирование публикации' : 'Новая публикация'}</h2>
              </div>
              <button type="button" onClick={() => !saving && setEditorOpen(false)} aria-label="Закрыть редактор" className="flex size-10 items-center justify-center border border-[#d7dde8] text-brand-navy hover:border-brand-orange">
                <X className="size-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 p-5 sm:p-8">
              ...
              <div className="flex flex-col-reverse gap-3 border-t border-[#dfe4ec] pt-6 sm:flex-row sm:justify-end">
                <button type="button" onClick={() => setEditorOpen(false)} disabled={saving} className="min-h-11 border border-[#cfd6e2] px-5 text-sm font-semibold text-brand-navy hover:border-brand-orange disabled:opacity-50">Отмена</button>
  ...
  return (
    <fieldset className="border border-[#dfe4ec] bg-white p-5 sm:p-6">
      ...
      <div className="space-y-5">
        ...
        <Field label="Текст статьи" required={required} hint="Разделяйте абзацы пустыми строками.">
