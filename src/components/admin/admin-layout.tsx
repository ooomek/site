...
const navigation = [
  { label: 'Exam results', to: '/admin/dashboard', icon: ClipboardCheck },
  { label: 'Question bank', to: '/admin/questions', icon: FileQuestion },
  { label: 'Blog', to: '/admin/blog', icon: BookOpenText },
];

export function AdminLayout({ title, description, eyebrow = 'Administration', actions, children }: AdminLayoutProps) {
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-brand-navy text-white">
      <div className="flex h-24 items-center border-b border-white/10 px-7">
        <a href="/?lang=ru" aria-label="Open MEK website" className="flex items-center gap-4">
      ...
      <div className="px-5 pt-8">
        <p className="px-3 text-[11px] font-semibold tracking-[0.18em] text-white/40 uppercase">Workspace</p>
